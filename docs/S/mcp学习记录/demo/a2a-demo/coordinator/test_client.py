    ...
    async def test_send_message(self, message: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=120.0) as client:
                ...
                send_message_payload: dict[str, Any] = {
                    'message': {
                        ...
                        'parts': [
                            {
                                'kind': 'text',
                                'text': json.dumps({  # 👈 用字符串包装 dict
                                    "start_time": '2025-06-30 12:00',
                                    "end_time": '2025-06-30 14:00'
                                }),
                            }
                        ],
                        'messageId': uuid4().hex,
                ...
                #
                #     response = await a2a_client.send_message(streaming_request)
                #
                #     print(response)
                #
...
@click.command()
@click.option('--url', default='http://localhost:8000', help='协调器Agent URL')
@click.option('--message', default='分析2025-06-30 12:00到2025-06-30 13:00之间的监控突刺', help='测试消息')
